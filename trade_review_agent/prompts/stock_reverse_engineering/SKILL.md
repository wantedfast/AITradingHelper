...
- Single stock: run all six roles for that stock. Also produce a concrete same-chain Three-High ranking whenever the research identifies any A-share company that is a cleaner expression of the profit pool than the input stock.
...
## Three-High Ranking Guardrail

For every single-stock task, the Three-High section must include:

1. `Input Stock Three-High Score`: score the input company.
2. `Same-Chain Core Asset Ranking`: rank concrete same-chain A-share companies discovered from the BOM, bottleneck, and profit-flow work.

This ranking is mandatory whenever any same-chain A-share company reaches at least medium evidence confidence. Sort by `Core Score = 0.4 * barrier + 0.3 * profit + 0.3 * growth`. Do not replace it with vague phrases such as `same-chain core assets`, `same-chain leaders`, or `cleaner expressions`. If none qualifies, explicitly say no same-chain A-share core asset was identified with medium confidence and explain the evidence gap.
