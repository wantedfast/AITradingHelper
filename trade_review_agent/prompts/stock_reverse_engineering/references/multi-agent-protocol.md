...
For every single-stock task, produce two Three-High tables:

1. `Input Stock Three-High Score`: score the input company.
2. `Same-Chain Core Asset Ranking`: rank concrete A-share companies discovered from the BOM, bottleneck, and profit-flow work that may express the profit pool better than the input company.

The `Same-Chain Core Asset Ranking` table is mandatory when any same-chain A-share company can be identified with at least medium confidence. Sort it by `Core Score = 0.4 * barrier + 0.3 * profit + 0.3 * growth`. Do not replace the ranking with vague phrases such as `same-chain core assets`, `same-chain leaders`, or `cleaner expressions`. If no same-chain company reaches medium confidence, explicitly state that and explain the evidence gap.
